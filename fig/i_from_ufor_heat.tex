	...
	\begin{tikzpicture}
	  ...
	  %          {$\pgfmathprintnumber[frac, frac denom=32,frac whole=false]{\pgfmathresult}\pi$}
		...
		legend style={
	     ...
	     % \xdef\C{5e-8}
	     % \xdef\R{13000}
		\addplot +[mark=*, line width=1pt, mark size=1pt, solid,ochre] plot [error bars/.cd, y dir = both, y explicit,x dir = both, x explicit] table [ x=uu, y=i] {../data/vax3.tsv};

		% \addplot[color=ochre,samples=1000, domain=0:80]
			% {2.19*25*ln(x/0.10)+x*0.38};
		    \end{axis}
		\end{tikzpicture}	
\end{document}